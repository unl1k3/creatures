...
The game uses a hybrid physics model. **Avian 2D** owns rigid, authored world
geometry; the blob itself remains a custom soft body. This separation keeps
platforms, ramps, hazards, nutrients, and safety boundaries stable while the
creature can deform organically.

### The deformable blob

Each blob is a closed membrane made of Verlet particles. A particle stores its
current and previous position rather than a conventional velocity. At the fixed
120 Hz simulation rate, the solver applies gravity and input, then iteratively
enforces several constraints:

- edge and second-neighbour distances preserve membrane continuity and bending;
- an area constraint preserves the creature's volume;
- stretch, collapse, and self-intersection guards prevent extreme shapes;
- localized deformation supports compression, breathing, digestion, shields,
  and other temporary organic effects.

Ground movement is not a rigid translation. Input applies traction near the
supporting membrane and a controlled angular response around the body centre.
Ground friction turns that response into rolling. Air control is weaker and
does not add intentional torque. Jumping first compresses the membrane and then
releases a size-scaled impulse along the measured support normal.

### Avian integration

Static level surfaces are Avian colliders created from the JSON level data.
The soft-body solver queries Avian contact manifolds, swept movement, surface
normals, and penetration depth for each membrane region. It then applies the
correction back to the affected Verlet particles, removes inward normal motion,
and records support/impact data. This prevents tunnelling through thin geometry
while preserving a soft visual contour instead of turning the blob into a rigid
circle.

Blob-to-blob and blob-to-corpse contacts use temporary convex Avian proxies
generated from their current membrane contours. Contact compression is shared
across nearby membrane points and centre-balanced, so a collision does not
create artificial translation or a sudden loss of shape.

### Water volumes

Wastewater is a gameplay volume rather than an Avian solid collider. Immersed
membrane points receive buoyancy, viscous drag, and a small transient flattening
that avoids a perfect circular silhouette. Without its shield, a blob mostly
drifts in water. With pseudo-spines extended (`Q`) and a horizontal direction,
the trailing immersed membrane contracts rhythmically: the spines act as
passive paddles, producing slow propulsion and a controlled roll.
...
animation stop immediately, while the body changes to the palette's light,
desaturated corpse grey and keeps its physical size and soft-body material.
There is no collapse or mineralization process. A corpse
...
Wastewater areas can optionally declare an `immune_family` index. The matching
blob family receives buoyancy and aquatic movement normally but no toxic damage;
all other living blob families are harmed while immersed. The standard level
includes a coral basin for family `1`, the colour created by the first split.
